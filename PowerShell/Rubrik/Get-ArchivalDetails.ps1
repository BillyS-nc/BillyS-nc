...
$serviceAccountPath = "L:\Billy\PowerShell_scripts\Rubrik\json\s-rubrik-powershell.json"

$date = Get-Date
$utcDate = $date.ToUniversalTime()

# CSV file info
$csvOutput = "L:\Billy\PowerShell_scripts\Rubrik\output\Retention\rubrik_archival_details-$($date.ToString("yyyy-MM-dd_HHmm")).csv"

# CSV to output file of objects without a recent archival to the 2nd location
# since the end of the prior month
$csvOutput2 = "L:\Billy\PowerShell_scripts\Rubrik\output\Retention\rubrik_archival_2nd_missing-$($date.ToString("yyyy-MM-dd_HHmm")).csv"
