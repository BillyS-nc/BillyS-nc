...
#$Servers= Get-Content "G:\PS_Scripts\servers.txt"
#$Sourcelist=$Dir$Servers 
...
$PingWrapper="PingWrapperLog"

$TempPSLogfile = "$($PingWrapper)_$(Get-Date -Format "yyyy-MM-dd_HH.mm.ss").txt"
...
####   #### 
#$Servers="G:\PS_Scripts\servers.txt"
...
foreach($server in $servers){
    ...
    #$pingtest = Test-Connection -ComputerName $server #-Quiet -Count 1 -ErrorAction SilentlyContinue
    $pingtest = ping $server | Add-Content "G:\PS_Scripts\PS_Script_Logs\pingresults.txt"
   # if($pingtest){
...
Get-ChildItem –Path $CleanupPath -Recurse | Where-Object {($_.LastWriteTime -lt (Get-Date).AddDays(-180))} | Remove-Item
