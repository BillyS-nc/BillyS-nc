...
####  NOTE: per Brn Seltzer, the Cohesity protectGenericNas.ps1 script is primarily intended to create new jobs not for modifying existing protection groups.
...
$CohPolicy="nas_test_15d"
$NewCohProtGrp="nas_TEST3_clmfilemounttst"
$NasMntPathList="\NAS_Adds.txt"
$NasExclusionList="\NAS_excludes.txt"
...
$RootCohProtGenNasWrapper="CohProtectGenNasWrapperLog"

$TempPSLogfile = "$($RootCohProtGenNasWrapper)_$(Get-Date -Format "yyyy-MM-dd_HH.mm.ss").txt"
...
Write-Host 'exclusions =' $Dir$NasExclusionList
...
#### run ./registerGenericNasList.ps1 with options    ####  -exclusions "/UTC"

./protectGenericNas.ps1 -vip $CohVip -username $CohUser -domain $CohDomain  -policyName $CohPolicy -jobName $NewCohProtGrp -mountList $Dir$NasMntPathList -timeZone 'America/New_York' -enableIndexing -inclusions "/dpsnap_" -exclusionList $Dir$NasExclusionList
...
Write-Host  'Cohesity Protection Policy used=' $CohPolicy
Write-Host  'adding objects to this Cohesity Protection Group=' $CohProtGrp
Write-Host  'source file used=' $Dir$NasMntPathList
Write-Host 'exclusions =' $Dir$NasExclusionList
