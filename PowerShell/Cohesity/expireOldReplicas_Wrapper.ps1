...
$CohVip="sbch-dp03br.sigi.us.selective.com"
$remoteCluster="sbch-dp01br.sigi.us.selective.com"
$CohUser="api-test"
$CohDomain="LOCAL"
$LogDir="\PS_Script_Logs\"
$CohProtGrp="sql_test_ACT-DB012T"
...
$RootexpireOldReplicasWrapper="CohexpireOldReplicasnWrapperLog"

$TempPSLogfile = "$($RootCohexpireOldReplicasWrapper)_$(Get-Date -Format "yyyy-MM-dd_HH.mm.ss").txt"
...
#                        -daysToKeep 14 `
#                        -remoteCluster 'othercluster' receiving replication
#                        -expire

#NOTE: data won't be deleted until you add the -expire paramter to your command.(if omitted, the script will only show what would be expired if including the expire option)

#./expireOldReplicas.ps1 -vip $CohVip -username $CohUser -domain $CohDomain -jobName $CohProtGrp -remoteCluster $remoteCluster #-expire

 
./expireOldReplicas.ps1 -vip $CohVip -username $CohUser -domain $CohDomain -jobName $CohProtGrp -remoteCluster $remoteCluster -daysToKeep $daysToKeep -daysBack $daysBack #-expire
