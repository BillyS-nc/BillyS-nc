...
$NsLookupWrapper="NsLookupWrapperLog"

$TempPSLogfile = "$($NsLookupWrapper)_$(Get-Date -Format "yyyy-MM-dd_HH.mm.ss").txt"
...
foreach($server in $servers){
    ...
    #$pingtest = Test-Connection -ComputerName $server #-Quiet -Count 1 -ErrorAction SilentlyContinue
    $pingtest = nslookup $server | Add-Content "G:\PS_Scripts\PS_Script_Logs\nslookupresults.txt"
